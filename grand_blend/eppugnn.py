...
class Eppugnn(InMemoryDataset):
    ...
    def process(self):
        ...
        if self.custom_split is not None:
            ...
            train_index, val_index = ShuffleSplit(n_splits=1, train_size=second_split_percentage).split(np.empty_like(labels[train_and_val_index]), labels[train_and_val_index])
            ...
            split = np.load(self.raw_dir + "/eppugnn_splits_" + self.name + "_0.6_0.2_"+ self.split_no + ".npz")
            data.train_mask = torch.from_numpy(split['train_mask']).to(torch.bool)
            data.val_mask = torch.from_numpy(split['val_mask']).to(torch.bool)
            data.test_mask = torch.from_numpy(split['test_mask']).to(torch.bool)
