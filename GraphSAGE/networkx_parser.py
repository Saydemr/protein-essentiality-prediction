...
np_adj_matrix = np.zeros((7349,7349), dtype=np.int8)
...
with open("BIOGRID-ORGANISM-Saccharomyces_cerevisiae_S288c-4.4.204.tab3.txt") as f:
    ...
    for line in f:
        ...
        if int(line[3]) not in id_map_int and int(line[4]) not in id_map_int:
            ...
            np_adj_matrix[id_map_int[int(line[3])]][i] = 1
            np_adj_matrix[i][id_map_int[int(line[3])]] = 1
            ...
            np_adj_matrix[i][id_map_int[int(line[4])]] = 1
            np_adj_matrix[id_map_int[int(line[4])]][i] = 1            
            ...
            np_adj_matrix[id_map_int[int(line[3])]][id_map_int[int(line[4])]] = 1
            np_adj_matrix[id_map_int[int(line[4])]][id_map_int[int(line[3])]] = 1
...
json.dump({str(v): int(k) for k, v in id_map.items()}, fp=open("../data/sc_eppugnn-id_map_inv.json", "w+"))
